...
export class SceneManager {
  ...
  setSelectedConnector(mesh) {
    ...
    if (mesh.userData && mesh.userData._isInstanceProxy) {
      try {
        ...
        logger.warn('No se pudo crear outline del conector instanciado', e);
      }
    ...
    if (mesh.geometry) {
      try {
        ...
        logger.warn('No se pudo crear outline del conector', e);
      }
    }
  ...
  setSelectedBeam(mesh) {
    ...
    if (mesh.geometry) {
      try {
        ...
        logger.warn('No se pudo crear outline de la viga', e);
      }
    }
  ...
  getConnectorOffsetPresetsMm(hit) {
    try {
      ...
      logger.warn('getConnectorOffsetPresetsMm fallo', e);
      return null;
    }
  }
  ...
  /**
   * Pausa/reanuda el loop de animación cuando el tab está oculto.
   * Ahorra CPU/GPU/batería en segundo plano.
   */
  _setupVisibilityPause() {
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        ...
        // Al volver, marcar dirty y reanudar
        this._needsRender = true;
        if (this._rafId == null) this.render();
      }
    });
  }
