...
export class PinwheelPDFReporter {
  ...
  static async generateReport(){
    ...
    // ── Páginas 2..N+1: tiras individuales (lineales, a escala máxima) ────
    for(let s=0;s<N;s++){
      doc.addPage();
      const strip=buildLinearStrip(s);
      this._drawStripPage(doc,strip,s,N,startK,totalLevels,colorByLevel);
    }

    // ── Última página: resumen ─────────────────────────────────────────────
    doc.addPage();
    this._drawSummaryPage(doc,N,startK,totalLevels,SCALE);
  ...
  static _drawPinwheelPage(doc,placed,scale,cx,cy,fcx,fcy,N,startK,totalLevels,colorByLevel){
    ...
    doc.text(
      `N=${stN}  α=${aDeg.toFixed(2)}°  ${cutActive?`Corte k=${cutLevel}`:'Sin corte'}  `+
      `${numStrips} secciones · ${facesPerStrip} caras/sección`,
      cx,15,{align:'center'});
    ...
    // Nota
    doc.setFontSize(6);doc.setFont(undefined,'italic');doc.setTextColor(120);
    doc.text('Sección 1 en centro-arriba. Sentido antihorario. Sin superposición.',cx,207,{align:'center'});
  }
  ...
  static _drawStripPage(doc,strip,sectionIdx,N,startK,totalLevels,colorByLevel){
    ...
    const offY=mt+(usH-sh*scale)/2 - mnY*1000*scale;
    ...
    doc.text(`Sección ${sectionIdx+1} de ${N}  (tira diagonal)`,PW/2,9,{align:'center'});
    doc.setFontSize(7);doc.setFont(undefined,'normal');doc.setTextColor(90);
    doc.text(
      `N=${stN}  α=${aDeg.toFixed(2)}°  ${cutActive?`Corte k=${cutLevel}`:'Sin corte'}  `+
      `k=${N-1}→k=${startK}  Escala 1:${(1/scale).toFixed(0)}`,
    ...
    for(const{verts2d,k,isTriangle}of strip){
      ...
      doc.text(`k${k}`,fx,fy+2,{align:'center'});
    }
